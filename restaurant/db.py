...
DATABASE_URL=os.getenv('DATABASE_URL','postgresql+psycopg://restaurante:restaurante@localhost:5432/meu_restaurante');engine=create_engine(DATABASE_URL,pool_pre_ping=True,future=True)
SCHEMA="""
...
CREATE TABLE IF NOT EXISTS tables_restaurant (id BIGSERIAL PRIMARY KEY,number INTEGER UNIQUE NOT NULL,seats INTEGER NOT NULL DEFAULT 4,status VARCHAR(30) NOT NULL DEFAULT 'FREE',opened_at TIMESTAMPTZ,waiter VARCHAR(120));
...
CREATE TABLE IF NOT EXISTS orders (id BIGSERIAL PRIMARY KEY,table_id BIGINT NOT NULL REFERENCES tables_restaurant(id),status VARCHAR(30) NOT NULL DEFAULT 'OPEN',created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),bill_requested_at TIMESTAMPTZ,closed_at TIMESTAMPTZ,payment_method VARCHAR(40));
...
CREATE TABLE IF NOT EXISTS payments (id BIGSERIAL PRIMARY KEY,order_id BIGINT NOT NULL REFERENCES orders(id),amount NUMERIC(12,2) NOT NULL,method VARCHAR(40) NOT NULL,provider VARCHAR(80),external_id VARCHAR(160),idempotency_key VARCHAR(160) UNIQUE,nsu VARCHAR(80),authorization_code VARCHAR(80),brand VARCHAR(80),status VARCHAR(30) NOT NULL DEFAULT 'PENDING',created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),approved_at TIMESTAMPTZ);
"""
MIGRATIONS=["ALTER TABLE orders ADD COLUMN IF NOT EXISTS bill_requested_at TIMESTAMPTZ","ALTER TABLE payments ADD COLUMN IF NOT EXISTS idempotency_key VARCHAR(160)","ALTER TABLE payments ADD COLUMN IF NOT EXISTS authorization_code VARCHAR(80)","ALTER TABLE payments ADD COLUMN IF NOT EXISTS brand VARCHAR(80)","ALTER TABLE payments ADD COLUMN IF NOT EXISTS approved_at TIMESTAMPTZ","CREATE UNIQUE INDEX IF NOT EXISTS ux_payments_idempotency ON payments(idempotency_key) WHERE idempotency_key IS NOT NULL"]
@contextmanager
def connect():
...
def init_db():
 ...
 with engine.begin() as con:
  ...
  for stmt in MIGRATIONS:con.execute(text(stmt))
  if con.execute(text('SELECT COUNT(*) FROM users')).scalar_one()==0:
   for n,u,p,r in [('Administrador','admin','admin123','ADM'),('Caixa','caixa','caixa123','CAIXA'),('Garçom','garcom','garcom123','GARCOM'),('Cozinha','cozinha','cozinha123','COZINHA')]:con.execute(text('INSERT INTO users(name,username,password_hash,role) VALUES (:n,:u,:p,:r)'),{'n':n,'u':u,'p':ph.hash(p),'r':r})
